...
row_list=$(grep -f  ${input_file}  /etc/hosts |  awk '{print $3}' | grep ^ATL | sed 's:[0-9]\|-: :g' | awk '{print $5}' | sort -u | tr -d '\r' | paste -sd,)
...
for rack in $(grep -f ${input_file} /etc/hosts | awk '{print $3}' | grep ^ATL | awk -F_ '{print $2 }' |  awk -F- '{print $NF}' | sort -u)
   do
	rackindex=$(get_rack_index ${rack})
	powercell=$(grep $rack /etc/hosts | awk '{print $3}' | sed 's:_:-:g' | awk -F- '{print $2}' | sort -u )
	cluster=$(grep $rack /etc/hosts | awk '{print $NF}' | awk -F- '{print $3}' | sort -u)
	PC_short=$(grep $rack /etc/hosts | awk '{print $NF}' | awk -F- '{print $2}' | sort -u)
